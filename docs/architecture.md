...
federal-mcps/
  ...
  infra/                  CDK: one Lambda + route per server, secrets, usage plans
...
| **M2 Geography catalog** | `packages/geography-build` pipeline; SQLite catalog (entity, alias, containment, agency_code, publishes_at, county_change); `resolvePlace` with ranking, ambiguity status and county fallback; `geography://guide` resource | "Denver" returns city, county, metro and CSA candidates with every BLS code and an ambiguity status; catalog rebuild is reproducible from source URLs in CI; resolver unit tests cover the gotchas in the geography spike |
