...
- An OpenContext ArcGIS connector on `https://data.hud.gov` (`arcgis__search_datasets`,
  ...
  **`arcgis__get_schema` currently fails on every dataset** (parse bug) and the connector
  **queries layer 0 only** — use the field names and layer notes below.
...
| Small Area FMRs (ZIP) | `6458c67bad2a4cc7aa97514ef7ba8a0e` | values are in **table layer 1** keyed `ID` = ZIP, `HUD_CODE`; layer 0 is ZCTA geometry only → **not reachable via the connector**; use FMR or the HUD SAFMR page | `SAFMR_2BR`, `*_90_Payment_Standard`, `*_110_Payment_Standard` | FY2026 |
...
| Low-Mod Income Population by Tract | `3bd6767dcc5e4937a6232d9db04dd447` | `GEOID` | `LOWMODPCT` (0–100) | its only layer is **id 4 → not reachable via the connector**; sum block groups or use the tract page |
...
| Opportunity Zones | `ef143299845841f8abb95969c01f88b5` | `GEOID10` (2010 tracts) | — | only layer is **id 13 → not reachable via the connector** |
...
- **Denver County designations:** QCT 2026 — 40 tracts (`STATE='08' AND COUNTY='031'`);
  Opportunity Zones — 10 (not via connector).
