variable "service_name" {
  ...
  type        = string
  default     = "rc-huduser-mcp"
}

variable "hud_user_token" {
  description = "HUD User Data API token (required for every data query once an indicator tool lands, ADR-018 §7), set on the Lambda as HUD_USER_TOKEN (ADR-006 §3)."
  type        = string
  sensitive   = true
}

variable "lambda_zip_path" {
  description = "Path to the esbuild bundle zip, catalog baked in (packages/server-hud/dist/lambda.zip in the instance root; a committed placeholder in this module's own tests)."
  type        = string
}
...
variable "domain_name" {
  description = "Custom hostname for this server (instances.json → domain.hudDomainName)."
  type        = string
}
