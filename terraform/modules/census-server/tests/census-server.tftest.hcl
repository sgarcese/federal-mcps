...
mock_provider "aws" {
  ...
  override_resource {
    ...
    values = {
      arn = "arn:aws:logs:us-east-1:123456789012:log-group:/aws/lambda/rc-census-mcp-dev"
    }
  }
  ...
  override_resource {
    ...
    override_during = plan
    values = {
      arn = "arn:aws:logs:us-east-1:123456789012:log-group:/aws/apigateway/rc-census-mcp-dev"
    }
  }
...
run "lambda_runtime_and_sizing" {
  ...
  assert {
    condition     = aws_lambda_function.census.function_name == "rc-census-mcp-dev"
    error_message = "function name must default to rc-census-mcp-dev"
  }

  assert {
    condition     = aws_lambda_function.census.runtime == "nodejs22.x"
    error_message = "runtime must be nodejs22.x"
  }

  assert {
    condition     = aws_lambda_function.census.architectures[0] == "arm64"
    error_message = "architecture must be arm64"
  }

  assert {
    condition     = aws_lambda_function.census.handler == "lambda.handler"
    error_message = "handler must be lambda.handler (the esbuild ESM bundle's export)"
  }

  assert {
    condition     = aws_lambda_function.census.memory_size == 512
    error_message = "memory must default to 512"
  }

  assert {
    condition     = aws_lambda_function.census.timeout == 29
...
run "lambda_environment_carries_transport_and_key" {
  ...
  assert {
    condition     = aws_lambda_function.census.environment[0].variables["MCP_TRANSPORT"] == "http"
    error_message = "MCP_TRANSPORT must be http"
  }

  # ADR-006 §3: the key rides as an environment variable from a sensitive variable.
  assert {
    condition     = aws_lambda_function.census.environment[0].variables["CENSUS_API_KEY"] == "test-key-value"
    error_message = "CENSUS_API_KEY must be set from var.census_api_key"
  }

  # #59, ADR-008 §7: the Census server resolves places off the bundled catalog.
  assert {
    condition     = aws_lambda_function.census.environment[0].variables["GEO_CATALOG_PATH"] == "/var/task/geo-catalog.sqlite"
    error_message = "GEO_CATALOG_PATH must point at the bundled catalog in the deployment package"
  }

  assert {
    condition     = !contains(keys(aws_lambda_function.census.environment[0].variables), "CENSUS_SECRET_ARN")
...
run "tracing_and_log_retention" {
  ...
  assert {
    condition     = aws_lambda_function.census.tracing_config[0].mode == "Active"
    error_message = "X-Ray tracing must be Active"
  }

  assert {
    condition     = aws_cloudwatch_log_group.lambda.name == "/aws/lambda/rc-census-mcp-dev"
...
run "lambda_uses_the_admin_provisioned_execution_role" {
  ...
  assert {
    condition     = data.aws_iam_role.exec.name == "rc-census-mcp-dev-role"
    error_message = "the Lambda must reference the <function_name>-role execution role by name"
  }

  assert {
    condition     = aws_lambda_function.census.role == data.aws_iam_role.exec.arn
...
run "http_api_routes_post_and_get_mcp_to_the_lambda" {
  ...
  assert {
    condition     = aws_apigatewayv2_api.census.protocol_type == "HTTP"
    error_message = "API must be an HTTP API"
  }

  assert {
    condition     = aws_apigatewayv2_integration.census.integration_type == "AWS_PROXY"
    error_message = "integration must be AWS_PROXY"
  }

  assert {
    condition     = aws_apigatewayv2_integration.census.payload_format_version == "2.0"
...
run "custom_domain_matches_the_instance_record" {
  ...
  assert {
    condition     = aws_acm_certificate.census.domain_name == "census-mcp.responsive.city"
    error_message = "certificate domain must equal var.domain_name"
  }

  assert {
    condition     = aws_acm_certificate.census.validation_method == "DNS"
    error_message = "certificate must use DNS validation"
  }

  assert {
    condition     = aws_apigatewayv2_domain_name.census.domain_name == "census-mcp.responsive.city"
    error_message = "apigatewayv2 domain name must equal var.domain_name"
  }

  assert {
    condition     = aws_apigatewayv2_domain_name.census.domain_name_configuration[0].endpoint_type == "REGIONAL"
    error_message = "domain endpoint must be REGIONAL"
  }

  assert {
    condition     = aws_apigatewayv2_domain_name.census.domain_name_configuration[0].security_policy == "TLS_1_2"
...
run "outputs_expose_the_invoke_urls" {
  ...
  assert {
    condition     = output.function_name == "rc-census-mcp-dev"
