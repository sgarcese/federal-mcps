# The Census server module (M8.1, ADR-014; ADR-004 §5-6, ADR-005 §1): Lambda + HTTP API +
...
resource "aws_lambda_function" "census" {
  ...
  environment {
    variables = {
      ...
      # The Census server resolves places off the bundled geography catalog (#59, ADR-008 §7),
...
resource "aws_apigatewayv2_api" "census" {
  name          = "${local.function_name}-api"
  protocol_type = "HTTP"
}

resource "aws_apigatewayv2_integration" "census" {
  api_id                 = aws_apigatewayv2_api.census.id
  integration_type       = "AWS_PROXY"
  integration_uri        = aws_lambda_function.census.invoke_arn
...
resource "aws_apigatewayv2_route" "post_mcp" {
  api_id    = aws_apigatewayv2_api.census.id
  route_key = "POST /mcp"
  target    = "integrations/${aws_apigatewayv2_integration.census.id}"
}
...
resource "aws_lambda_permission" "apigw" {
  ...
  function_name = aws_lambda_function.census.function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_apigatewayv2_api.census.execution_arn}/*/*"
}

# --- Custom domain ---------------------------------------------------------

resource "aws_acm_certificate" "census" {
...
resource "aws_route53_record" "cert_validation" {
  for_each = {
    for dvo in aws_acm_certificate.census.domain_validation_options : dvo.domain_name => {
...
resource "aws_acm_certificate_validation" "census" {
  certificate_arn         = aws_acm_certificate.census.arn
  validation_record_fqdns = [for r in aws_route53_record.cert_validation : r.fqdn]
}

resource "aws_apigatewayv2_domain_name" "census" {
  domain_name = var.domain_name

  domain_name_configuration {
    certificate_arn = aws_acm_certificate_validation.census.certificate_arn
    endpoint_type   = "REGIONAL"
    security_policy = "TLS_1_2"
  }
}

resource "aws_apigatewayv2_api_mapping" "census" {
  api_id      = aws_apigatewayv2_api.census.id
  domain_name = aws_apigatewayv2_domain_name.census.id
...
resource "aws_route53_record" "alias_a" {
  ...
  alias {
    name                   = aws_apigatewayv2_domain_name.census.domain_name_configuration[0].target_domain_name
    zone_id                = aws_apigatewayv2_domain_name.census.domain_name_configuration[0].hosted_zone_id
