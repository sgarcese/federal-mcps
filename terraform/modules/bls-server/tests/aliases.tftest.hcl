...
mock_provider "aws" {
  ...
  override_resource {
    target          = aws_acm_certificate.alias["bls.responsive.city"]
    override_during = plan
    values = {
      arn = "arn:aws:acm:us-east-1:123456789012:certificate/test-cert-id-alias"
      domain_validation_options = [{
        domain_name           = "bls.responsive.city"
        resource_record_name  = "_acme-challenge.bls.responsive.city."
        resource_record_type  = "CNAME"
        resource_record_value = "example.acm-validations.aws."
      }]
    }
...
variables {
  ...
  domain_name        = "bls-mcp.responsive.city"
  alias_domain_names = ["bls.responsive.city"]
...
run "an_alias_gets_its_own_certificate_domain_mapping_and_records" {
  ...
  assert {
    condition     = aws_acm_certificate.alias["bls.responsive.city"].domain_name == "bls.responsive.city"
    error_message = "the alias must get its own DNS-validated certificate"
  }
  assert {
    condition     = aws_apigatewayv2_domain_name.alias["bls.responsive.city"].domain_name == "bls.responsive.city"
    error_message = "the alias must get its own apigatewayv2 domain name"
  }
  assert {
    condition     = aws_apigatewayv2_api_mapping.alias["bls.responsive.city"].api_id == aws_apigatewayv2_api.bls.id
    error_message = "the alias must map to the same API"
  }
  assert {
    condition     = aws_route53_record.alias_alias_a["bls.responsive.city"].zone_id == "ZTESTZONE" && aws_route53_record.alias_alias_aaaa["bls.responsive.city"].type == "AAAA"
...
run "the_primary_domain_is_untouched_by_aliases" {
  ...
  assert {
    condition     = aws_acm_certificate.bls.domain_name == "bls-mcp.responsive.city" && aws_apigatewayv2_domain_name.bls.domain_name == "bls-mcp.responsive.city"
  ...
  assert {
    condition     = output.alias_urls == ["https://bls.responsive.city/mcp"]
