...
// The adapter opens the catalog at import (createHudServer runs at module top level),
...
describe("hud lambda handler", () => {
  it("answers initialize with 200 and the hud server name", async () => {
    const result = await handler(rpcEvent("initialize", INITIALIZE_PARAMS));
    expect(result.statusCode).toBe(200);
    const body = JSON.parse((result.body as string) ?? "{}");
    expect(body.result.serverInfo.name).toBe("federal-mcps-hud");
  });

  it("lists hud_resolve_place and hud_describe_source on tools/list", async () => {
    await handler(rpcEvent("initialize", INITIALIZE_PARAMS));
    const result = await handler(rpcEvent("tools/list"));
    const body = JSON.parse((result.body as string) ?? "{}");
    const names = body.result.tools.map((tool: { name: string }) => tool.name);
    expect(names).toContain("hud_resolve_place");
    expect(names).toContain("hud_describe_source");
  });
