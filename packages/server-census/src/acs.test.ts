...
describe("buildAcsQueryUrl (must match scripts/record-acs-fixtures.mts exactly)", () => {
  it("asks for E, M, EA and MA of one variable by ucgid", () => {
    expect(
      buildAcsQueryUrl({ vintage: "2024", product: "5-year", family: "subject", variable: "S1701_C03_001", ucgid: "0500000US08031" }),
    ).toBe(
    ...
    expect(buildAcsQueryUrl({ vintage: "2024", product: "1-year", family: "detailed", variable: "B01003_001", ucgid: "1600000US0820000" })).toBe(
...
describe("chooseProduct (ADR-014 §2)", () => {
  it("picks 1-year at 65,000+ and 5-year below, tracts and ZCTAs always 5-year", () => {
    expect(chooseProduct({ population: 729_019, sumlevel: "160" })).toEqual({ product: "1-year", reason: "population 729,019 is at or above the ACS 1-year threshold of 65,000" });
  ...
  it("honours an explicit 5-year request, and answers a 1-year request below the threshold with 5-year plus a reason", () => {
    expect(chooseProduct({ population: 729_019, sumlevel: "160", requested: "5-year" }).product).toBe("5-year");
...
describe("parseAcsResponse: sentinels, annotations, open-ended medians (ADR-014 §4)", () => {
  const row = (e: string | null, m: string | null, ea: string | null, ma: string | null) =>
    JSON.stringify([["NAME", "B19013_001E", "B19013_001M", "B19013_001EA", "B19013_001MA", "ucgid"], ["X", e, m, ea, ma, "u"]]);
  it("reads a plain estimate and margin", () => {
    expect(parseAcsResponse(row("73738", "12737", null, null), "B19013_001")).toMatchObject({ value: 73738, marginOfError: 12737, caveats: [] });
  });
  ...
  it("keeps a controlled estimate's value with a null margin and a note", () => {
    const r = parseAcsResponse(row("729019", "-555555555", null, "*****"), "B01003_001");
...
describe("acsFetch over recorded fixtures", () => {
  const key = (p: "1-year" | "5-year", family: "detailed" | "subject", variable: string, ucgid: string) =>
  ...
  it("round-trips a series key", () => {
    ...
    expect(parseAcsSeriesKey(k)).toEqual({ vintage: "2024", product: "5-year", family: "subject", variable: "S1701_C03_001", ucgid: "0500000US08031" });
  });

  it("returns Denver city's 1-year population with the controlled-estimate note and no margin", async () => {
    const [r] = await acsFetch(replay(), [key("1-year", "detailed", "B01003_001", "1600000US0820000")], {});
    expect(r?.observations[0]).toMatchObject({ year: "2024", period: "A01", value: 729019, marginOfError: null });
  ...
  it("returns Denver city's 1-year median income with margin and a high reliability grade", async () => {
    const [r] = await acsFetch(replay(), [key("1-year", "detailed", "B19013_001", "1600000US0820000")], {});
    expect(r?.observations[0]).toMatchObject({ value: 92504, marginOfError: 3897, reliability: "high" });
  });

  it("falls to the 5-year product when the 1-year query answers 204, and says so", async () => {
    const [r] = await acsFetch(replay(), [key("1-year", "detailed", "B01003_001", "1600000US0465350")], {});
    expect(r?.observations[0]).toMatchObject({ value: 9777, period: "5Y", periodName: "2020–2024" });
    expect(r?.observations[0]?.footnotes.map((f) => f.text).join(" ")).toMatch(/1-year.*not published/i);
  });

  it("returns a percent from a subject table and a low-reliability tract with its grade", async () => {
    const [pct] = await acsFetch(replay(), [key("5-year", "subject", "S1701_C03_001", "0500000US08031")], {});
    expect(pct?.observations[0]).toMatchObject({ value: 11.2, marginOfError: 0.5, reliability: "high" });
    const [low] = await acsFetch(replay(), [key("5-year", "detailed", "B19013_001", "1400000US08031000503")], {});
    expect(low?.observations[0]).toMatchObject({ value: 121379, marginOfError: 82893, reliability: "low" });
  });

  it("returns null plus the Census meaning for an uncomputable tract median", async () => {
    const [r] = await acsFetch(replay(), [key("5-year", "detailed", "B19013_001", "1400000US08031980001")], {});
    expect(r?.observations[0]?.value).toBeNull();
    expect(r?.observations[0]?.footnotes.map((f) => f.text).join(" ")).toMatch(/could not be computed/);
  });
});
