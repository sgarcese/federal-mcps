/**
 * Local HTTP dev server: `npm run dev:http -w packages/server-census`.
 *
 * Serves `createCensusHttpHandler()` on plain `node:http` so the Streamable HTTP
...
import { createCensusHttpHandler, MCP_PATH } from "./http.js";
...
const handler = createCensusHttpHandler();
const server = createNodeHttpServer((req, res) => {
  void handler(req, res);
});

server.listen(port, () => {
  process.stderr.write(`federal-mcps-census listening on http://localhost:${port}${MCP_PATH}\n`);
});
