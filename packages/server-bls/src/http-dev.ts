/**
 * Local HTTP dev server: `npm run dev:http -w packages/server-bls`.
 *
 * Serves `createBlsHttpHandler()` on plain `node:http` so the Streamable HTTP
...
const handler = createBlsHttpHandler();
const server = createNodeHttpServer((req, res) => {
  void handler(req, res);
});

server.listen(port, () => {
  process.stderr.write(`federal-mcps-bls listening on http://localhost:${port}${MCP_PATH}\n`);
});
